...
- **MMDevice endpoint IDs are NOT stable for NVIDIA HDA DP/HDMI outputs** — the driver mints a new endpoint GUID across reboots/display re-enumeration (USB/Bluetooth IDs are stable). Never compare a persisted `EndpointId` against live devices directly; go through `DeviceMatcher.HealEndpointIds` (display-name fallback + self-heal), the single match entry point used by `CycleController`, `SettingsForm`, and the tray label refresh.
...
- **System.Text.Json does not enforce non-null annotations** — a hand-edited `audio-carousel.json` can carry null lists/keys through deserialization. Config-consuming code must degrade gracefully instead of assuming non-null (see `HotkeyParser.FromConfigEntry` and its null-tolerance tests).
...
- **Publish smoke test**: after running `publish.ps1`, the exe lives in `publish/`. If the previous instance is still running, `Remove-Item publish/` fails. Always `taskkill.exe //F //IM AudioCarousel.exe` first. `publish.ps1` wipes the whole `publish/` dir — back up `publish/audio-carousel.json` first and restore it after if the existing config matters (it may be a live config; see CLAUDE.local.md if present). `publish/` is gitignored, so nothing there can leak into a commit.
- **THIRD_PARTY_NOTICES.md pins exact dependency versions** — Dependabot bumps do not touch it; update it manually whenever a package version changes.
- **Releases**: pushes to `main` only run `ci.yml` (build/test/format) — they never touch the Releases page. To cut a release, tag `v*.*.*` and push the tag. `.github/workflows/release.yml` strips the `v` prefix and passes the version as `./scripts/publish.ps1 -Version <ver>`, which forwards `-p:Version=<ver>` to `dotnet publish` to override the `1.0.0-dev` placeholder in csproj. The build is then `Compress-Archive`d into `AudioCarousel-<ver>-win-x64.zip` (~43 MB, ~60% smaller than the raw exe) and attached to the GitHub Release automatically. Auto-generated release notes list only merged PRs (i.e. Dependabot) — changes committed directly to `main` are invisible. After the Release workflow finishes, rewrite the notes with `gh release edit <tag> --notes-file -`.
