...
Audio Carousel 调用 `IPolicyConfig` 这一**未公开的 Windows COM 接口**来切换默认音频终结点。许多同类工具（SoundSwitch、EarTrumpet、NirCmd 等）都采用相同手法，从 Windows Vista 到 Windows 11 一直稳定可用。但由于该 API 是非官方的，微软可能在未来的 Windows 更新中无预警地更改或移除它。如果未来的 Windows 更新导致 Audio Carousel 无法工作，请提交 Issue。

发布的二进制文件未经代码签名（为免费工具购买签名证书需要实际花费）。在二进制声誉积累起来之前，Windows SmartScreen 会在首次启动时发出警告——参见 **使用方法** 中的 *首次运行提示*。
